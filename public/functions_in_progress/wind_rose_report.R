...
wind.rose.report = function(sites, bgn.month, end.month, min.speed=0.5, save.dir, qf.plot="ALL",
                            summarize=FALSE, show.boom=FALSE){
    options(stringsAsFactors = FALSE)
    DirCut=NULL
    SpeedCut=NULL

    print(sites)
    if(!as.character(qf.plot) %in% c("0", "1", "ALL")){
        message("Plotting all QF types. Please specify 0 or 1 to subset on a QF value.")
        qf.plot="ALL"}
    ...
    if(as.character(qf.plot)=="0"){
       save.name="wind_directions_qf0.pdf"
    }
    if(as.character(qf.plot)=="1"){
        save.name="wind_directions_qf1.pdf"
    }
    if(as.character(qf.plot)=="ALL"){
        save.name="wind_directions.pdf"
    }

    grDevices::pdf(file = paste0(save.dir, "/", save.name), onefile = T, paper = "US", width = 10, height = 10)
    ...
    for(s in 1:length(sites)){
        ...
        # Return data
        data=try(Noble::pull.data(site = site, dp.id = "DP1.00001.001", bgn.month = bgn.month, end.month = end.month, time.agr = 30, package="basic", save.dir = tempdir()))
        if(class(data)=="try-error"){
            ...
            for(n in 1:length(mls)){
                ...
                temp.df<-data.frame(Date=data[,1], "Dir"=as.numeric(direct),
                                    "Speed"=as.numeric(speed), "QualityFlag"=qf, "ML"=rep(clean.mls[n], length(direct)))
                ...
                sum.seasons=c("Fall", "Spring", "Summer", "Winter")
                all.pts=table(clean.df[,c("Season", "ML")])
                no.lite.df=clean.df[clean.df$Speed>0.5,]
                str.pts=table(no.lite.df[,c("Season", "ML")])
                if(!length(all.pts)==length(str.pts)|length(all.pts)<4){

                    all.miss.seas=sum.seasons[!sum.seasons %in% rownames(all.pts)]
                    str.miss.seas=sum.seasons[!sum.seasons %in% rownames(str.pts)]
                    all.diff=length(all.miss.seas)
                    str.diff=length(str.miss.seas)
                    rbind(c(str.pts, rep(0, times=str.diff))) %>%
                        as.numeric %>%
                        `names<-`(c(rownames(str.pts), str.miss.seas)) ->
                        str.test
                    rbind(c(all.pts, rep(0, times=all.diff))) %>%
                        as.numeric %>%
                        `names<-`(c(rownames(all.pts), all.miss.seas)) ->
                        all.test
                    all.test=all.test[order(names(all.test))]
                    str.test=str.test[order(names(str.test))]
                    str.pts=str.test
                    all.pts=all.test
                }
            ...
            subtitle=paste0("\nDirection convention: blowing from. \nPercent of calm winds (", min.speed, " m/s or lower): \n ",
                            paste0(clean.mls,": ",  ml.avgs, "%", collapse = "\n  "))
            ...
            subtitle=paste0("Lat/Lon: ", lat, ", ", lon, subtitle)
            ...
            plot=ggplot2::ggplot(data = all.binned, ggplot2::aes(x=DirCut, fill=SpeedCut, colors=factor(SpeedCut)))+
                ...
                #ggplot2::geom_rect(mapping = ggplot2::aes_(xmin = low, xmax = high, ymin = 0, ymax = ymax*1.05), fill="red",  alpha = 0.1, show.legend = T)



            if(as.character(qf.plot)=="0"){
                plot=plot+ggplot2::ggtitle(label = titleString, subtitle = paste0(subtitle, "\nOnly showing records where QF=0"))

            }
            if(as.character(qf.plot)=="1"){
                plot=plot+ggplot2::ggtitle(label = titleString, subtitle = paste0(subtitle, "\nOnly showing records where QF=1"))
            }
            if(as.character(qf.plot)=="ALL"){
