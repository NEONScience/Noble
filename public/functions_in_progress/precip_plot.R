precip.plot=function(site, bgn.month, end.month, cumulative=T){
    ...
    if(type=="Core"){
        precip=Noble::pull.data(site = site, dp.id = "DP1.00006.001", bgn.month = bgn.month, end.month = end.month, time.agr = 5, package = "basic", save.dir = tempdir(), complete.times = F)
    }
    if(type=="Relocatable"){
        precip=Noble::pull.data(site = site, dp.id = "DP1.00006.001", bgn.month = bgn.month, end.month = end.month, time.agr = 1, package = "basic", save.dir = tempdir(), complete.times = F)
    }
